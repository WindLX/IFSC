...
if __name__ == "__main__":
    ...
    results = {"cluster_num": [], "uav_num": [], "max_uav_num": []}
    for max_uav_num in [15, 20, 25]:
        ...
        for uav_num in tqdm(range(50, 251, 50), desc=f"max_uav_num={max_uav_num}"):
            ...
            uav_swarm.reset()
            uav_swarm.update()
            results["cluster_num"].append(uav_swarm.cluster_numbers)
            results["uav_num"].append(uav_num)
            results["max_uav_num"].append(max_uav_num)

    df = pd.DataFrame(results)
    plt.figure(figsize=(10, 6))
    sns.barplot(x="uav_num", y="cluster_num", hue="max_uav_num", data=df)
    plt.xlabel("Number of UAVs")
    plt.ylabel("The number of clusters")
    plt.title("Number of clusters vs Number of UAVs for Different max_uav_num")
    plt.legend(title="max_uav_num")
    plt.savefig("image/cluster_number_vs_uav_num_for_max_uav_num.png")
    plt.show()
